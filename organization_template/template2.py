...
# Example commands to showcase what's happening.
# Why use cogs when you can just use python's built in library loading system and just import the code/functions instead.
...
@tmp.tree.command(description='Does addition.')
async def add(interaction: discord.Interaction, number1: int, number2: int):
    from Commands import math as mcmd
    # numbesr2 = str(number2) #this is just to show you that it can handle errors too. (WILL cause an error)
    check, _ = await mcmd.add(interaction, number1, number2)
    if check == 000:
        await interaction.response.send_message(f"{_}", ephemeral=True, delete_after=10)
